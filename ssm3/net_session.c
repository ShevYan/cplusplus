	...
	do {
		...
		case SPRINT_MSG_LEN:
			...
			if (s->cur_pos + buf + len - p >= s->cur_size) {
				memcpy((char *) &s->cur_msg->msglen + s->cur_pos, p,
						s->cur_size - s->cur_pos);
				s->cur_msg->msglen = htobe32(s->cur_msg->msglen);
				p += s->cur_size - s->cur_pos;
		...
		case SPRINT_CRC:
			if (s->cur_pos + buf + len - p >= s->cur_size) {
				...
				s->cur_msg->crc = htobe32(s->cur_msg->crc);
				p += s->cur_size - s->cur_pos;
		...
		case SPRINT_MAGIC:
			if (s->cur_pos + buf + len - p >= s->cur_size) {
				memcpy((char *) &s->cur_msg->magic + s->cur_pos, p,
						s->cur_size - s->cur_pos);
				s->cur_msg->magic = htobe32(s->cur_msg->magic);
				p += s->cur_size - s->cur_pos;
