...
#define MCD_PRINTF wprintf
#define MCD_SPRINTF swprintf
#define MCD_SNPRINTF _snwprintf
#define MCD_VSNPRINTF _vsnwprintf
#define MCD_FOPEN(f,n,m) f=_wfopen(n,m)
#define MCD_TTOI _wtoi
#define MCD_REMOVEFILE _wremove
#define MCD_RENAME _wrename
#endif // not VC++ safe strings
#define MCD_T(s) L ## s
...
#define MCD_PRINTF printf_s
#define MCD_SPRINTF sprintf_s
#define MCD_SNPRINTF _snprintf_s
#define MCD_VSNPRINTF vsnprintf_s
#define MCD_FOPEN(f,n,m) {if(fopen_s(&f,n,m)!=0)f=NULL;}
#define MCD_REMOVEFILE remove
#define MCD_RENAME rename
#else // not VC++ safe strings
#define MCD_SSZ(sz) sz
...
#define MCD_SNPRINTF snprintf
#define MCD_VSNPRINTF vsnprintf
#define MCD_FOPEN(f,n,m) f=fopen(n,m)
#define MCD_TTOI atoi
#define MCD_REMOVEFILE remove
#define MCD_RENAME rename
#endif // not VC++ safe strings
#define MCD_T(s) s
...
#define MCD_STRMID2(s,n) s.substr((n))
...
#define MCD_STRFIND(s,c,n) (s).find((c),(n))
#define MCD_STRFIND2(s,c) (s).find((c))
#define MCD_STRDEL(s,p,n) s.erase((p),(n))
#define MCD_INSERT(s,p,n) s.insert((p),(n))
#define MCD_STRNPOS MCD_STR::npos
#else // not STL, i.e. MFC
// afx.h provides string, to avoid "WINVER not defined" #include stdafh.x in Markup.cpp
//#include <afx.h>
#include <atlstr.h>
...
#define MCD_STRMID2(s,n) s.Mid((n))
...
#define MCD_STRFIND(s,c,n) (s).Find((c),(n))
#define MCD_STRFIND2(s,c) (s).Find((c))
#define MCD_STRDEL(s,p,n) s.Delete((p),(n))
#define MCD_INSERT(s,p,n) s.Insert((p),(n))
#define MCD_STRNPOS (-1)
#endif // not STL
